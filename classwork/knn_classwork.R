...
##Create KNN Function
# n for the number of folds
# maximum k for the maximum nearest neighbors
knn.nfold <- function(n, maximum.k){
...
# train.pct <- .7
# Set training percentage accordingly with number of folds
train.pct <- (n-1)/n

train.index <- sample(1:N, train.pct * N)       # random sample of records (training set)

#Get a new training set for each run. The tests sets may overlap slightly in this case.
train.index <- (train.index + i) %% N
...
## Print statement to check the looping
cat('\n', 'run number: ', i)
